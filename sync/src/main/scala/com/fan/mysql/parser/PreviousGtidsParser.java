...
import com.dingcloud.dts.binlog.event.impl.PreviousGtidsEvent;
import com.dingcloud.dts.binlog.mysql.dbsync.BinlogContext;
import com.dingcloud.dts.binlog.mysql.dbsync.LogBuffer;
import com.dingcloud.dts.binlog.position.GtidSetPosition;
import com.dingcloud.dts.binlog.position.GtidSetPosition.Interval;
import com.dingcloud.dts.binlog.position.GtidSetPosition.UUIDSet;
...
public class PreviousGtidsParser implements BinlogEventParser {
	...
	public static byte[] getBody(String gtidSet) {
		...
		// calculate body size
		int bodySize = 8;
		Collection<UUIDSet> uuids = pos.getUUIDSets();
		for (UUIDSet uuidSet : uuids) {
			bodySize += 16 + 8;
		...
		for (UUIDSet uuidSet : uuids) {
			...
			offset += 8;
			for (Interval interval : uuidSet.getIntervals()) {
