package com.dingcloud.dts.binlog.util;
...
public class JsonConversion {
	...
	private static Json_Value parse_array_or_object(Json_enum_type type, LogBuffer buffer, long len, boolean large) {
		...
		buffer.rewind();
		return new Json_Value(type, buffer, element_count, bytes, large);
	}
	...
	private static Json_Value parse_scalar(int type, LogBuffer buffer, long len) {
		...
		case JSONB_TYPE_STRING:
			...
			buffer.rewind();
			buffer.forward((int) n);
			return new Json_Value(Json_enum_type.STRING, buffer.getFixString((int) str_len));
		case JSONB_TYPE_OPAQUE:
			...
			buffer.forward((int) q_n);
			return new Json_Value(type_byte, buffer, q_str_len);
		default:
			throw new IllegalArgumentException("illegal json data");
		}
	}
	...
	public static class Json_Value {
		...
		public String key(int i) {
			...
			m_data.rewind();
			m_data.forward((int) key_offset);
			return m_data.getFixString((int) key_length);
		}
			...
			case OPAQUE:
				...
				if (m_field_type == MySQLConstants.MYSQL_TYPE_NEWDECIMAL) {
					...
					if (packed_value == 0) {
						...
						text = String.format("%s%02d:%02d:%02d", packed_value >= 0 ? "" : "-",
								(int) ((intpart >> 12) % (1 << 10)), (int) ((intpart >> 6) % (1 << 6)),
								(int) (intpart % (1 << 6)));
						text = text + "." + usecondsToStr(frac, 6);
					}
					buf.append('"').append(text).append('"');
				} else if (m_field_type == MySQLConstants.MYSQL_TYPE_DATE
						|| m_field_type == MySQLConstants.MYSQL_TYPE_DATETIME
						|| m_field_type == MySQLConstants.MYSQL_TYPE_TIMESTAMP) {
					long packed_value = m_data.getLong64();
					if (packed_value == 0) {
						...
						text = String.format("%04d-%02d-%02d %02d:%02d:%02d", (int) (ym / 13), (int) (ym % 13),
				...
				break;
			case STRING:
				m_string_value = m_string_value.replaceAll("\\\\", "\\\\\\\\").replaceAll("\"", "\\\\\"");
