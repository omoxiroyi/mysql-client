...
class MySQLConnectionHandler(
  ...
  private[this] final val decoder: MySQLPacketDecoder =
    new MySQLPacketDecoder(configuration.charset, connectionId)
  private[this] final val encoder: MySQLOneToOneEncoder =
  ...
  private[this] final val connectionPromise: Promise[MySQLConnectionHandler] =
    Promise[MySQLConnectionHandler]
  ...
  def connect(): Future[MySQLConnectionHandler] = {
    ...
    this.bootstrap
      .option[ByteBufAllocator](ChannelOption.ALLOCATOR, LittleEndianByteBufAllocator.INSTANCE)
      ...
      case m: ServerMessage =>
        (m.kind: @switch) match {
          ...
          case ServerMessage.BinaryRow =>
            ...
            this.currentQuery
              .addRow(this.binaryRowDecoder.decode(message.buffer, this.currentColumns.toSeq))
  ...
  private def executePreparedStatement(
    ...
    if (longValues.nonEmpty) {
      ...
      channelFuture flatMap { _ =>
        writeAndHandleError(
          PreparedStatementExecuteMessage(statementId, values, nonLongIndices.toSet, parameters)
        )
      }
    } else {
      writeAndHandleError(
  ...
  private def onPreparedStatementPrepareResponse(message: PreparedStatementPrepareResponse) {
    this.currentPreparedStatementHolder =
      new PreparedStatementHolder(this.currentPreparedStatement.statement, message)
  }
  ...
  def onColumnDefinitionFinished(): Unit = {
    ...
    if (this.currentPreparedStatementHolder != null) {
      this.parsedStatements
        .put(this.currentPreparedStatementHolder.statement, this.currentPreparedStatementHolder)
      this.executePreparedStatement(
