...
class DecoderRegistry(charset: Charset) {
  ...
  def binaryDecoderFor(columnType: Int, charsetCode: Int): BinaryDecoder = {
    ...
    (columnType: @switch) match {
      case ColumnTypes.FIELD_TYPE_VARCHAR | ColumnTypes.FIELD_TYPE_ENUM => this.stringDecoder
      case ColumnTypes.FIELD_TYPE_BLOB | ColumnTypes.FIELD_TYPE_LONG_BLOB |
          ColumnTypes.FIELD_TYPE_MEDIUM_BLOB | ColumnTypes.FIELD_TYPE_TINY_BLOB |
          ColumnTypes.FIELD_TYPE_VAR_STRING | ColumnTypes.FIELD_TYPE_STRING =>
  ...
  def textDecoderFor(columnType: Int, charsetCode: Int): ColumnDecoder = {
    (columnType: @switch) match {
      ...
      case ColumnTypes.FIELD_TYPE_VARCHAR | ColumnTypes.FIELD_TYPE_ENUM => StringEncoderDecoder
